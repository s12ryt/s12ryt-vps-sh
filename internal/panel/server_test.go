...
func TestDashboardProvidesModeTopologyAndProtocolConfiguration(t *testing.T) {
	...
	for _, expected := range []string{
		...
		`data-listener-field-group`,
		`id="node-listener-error" data-node-listener-error role="alert"`,
		`aria-describedby="node-listener-help node-listener-error"`,
		`setNodeListenerError`,
		`aria-invalid`,
		...
		`aria-busy`,
		`inFlightOperations`,
		`dashboardBackground.inert`,
		`[data-modal]:not([hidden])`,
		`scope.matches('button,input,select,textarea')`,
		`[data-config-save],[data-node-delete],[data-remote-toggle],[data-remote-delete]`,
