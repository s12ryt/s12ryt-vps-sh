...
func (server *Server) showPanel(response http.ResponseWriter, request *http.Request) {
	...
	rendered = strings.Replace(rendered, `</script>`, workspaceNavigationScript+modalAccessibilityScript+operationStateScript+nodeManagementScript+nodeDeploymentScript+remoteManagementScript+networkManagementScript+shareManagementScript+`</script>`, 1)
