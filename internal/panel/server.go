...
func (server *Server) showPanel(response http.ResponseWriter, request *http.Request) {
	...
	rendered := strings.Replace(
		page.String(),
		`<main class="shell"`,
		`<main class="shell" data-nodes-endpoint="`+html.EscapeString(server.basePath+"/api/nodes")+`" data-credential-endpoint-template="`+html.EscapeString(server.basePath+"/api/nodes/{id}/credential")+`" data-remotes-endpoint="`+html.EscapeString(server.basePath+"/api/remotes")+`" data-ipv4-fallback-endpoint="`+html.EscapeString(server.basePath+"/api/ipv4-fallback")+`"`,
		1,
	)
	rendered = strings.Replace(rendered, `</section></main>`, nodeWorkspaceHTML+remoteWorkspaceHTML+`</section></main>`, 1)
	rendered = strings.Replace(rendered, `<script>`, nodeModalsHTML+remoteModalHTML+`<script>`, 1)
	...
	rendered = strings.Replace(rendered, `</script>`, nodeManagementScript+nodeDeploymentScript+remoteManagementScript+`</script>`, 1)
