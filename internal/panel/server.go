...
func (server *Server) showPanel(response http.ResponseWriter, request *http.Request) {
	...
	rendered = strings.Replace(rendered, `<body><main class="shell"`, `<body><div class="dashboard-frame">`+workspaceNavigationHTML+`<main id="workspace-content" class="shell"`, 1)
	...
	rendered = strings.Replace(rendered, `</script>`, workspaceNavigationScript+nodeManagementScript+nodeDeploymentScript+remoteManagementScript+networkManagementScript+shareManagementScript+`</script>`, 1)
