...
func (server *Server) showPanel(response http.ResponseWriter, request *http.Request) {
	...
	rendered := strings.Replace(page.String(), `</head>`, mobileDashboardStyle+`</head>`, 1)
	...
	rendered = strings.Replace(rendered, `</section></main>`, nodeWorkspaceHTML+remoteWorkspaceHTML+networkWorkspaceHTML+shareWorkspaceHTML+`</section></main>`, 1)
	...
	rendered = strings.Replace(rendered, `</script>`, nodeManagementScript+nodeDeploymentScript+remoteManagementScript+networkManagementScript+shareManagementScript+`</script>`, 1)
