...
func (server *Server) showPanel(response http.ResponseWriter, request *http.Request) {
	...
	rendered := strings.Replace(page.String(), `</head>`, nocDashboardStyle+mobileDashboardStyle+`</head>`, 1)
	...
	rendered = strings.Replace(rendered, `<body><main class="shell"`, `<body><div class="operation-notice" data-operation-notice role="status" aria-live="polite" aria-atomic="true" hidden></div><div class="dashboard-frame">`+workspaceNavigationHTML+`<main id="workspace-content" class="shell"`, 1)
	...
	rendered = strings.Replace(rendered, `</script>`, workspaceNavigationScript+operationStateScript+nodeManagementScript+nodeDeploymentScript+remoteManagementScript+networkManagementScript+shareManagementScript+`</script>`, 1)
...
;const operationNotice=document.querySelector('[data-operation-notice]');const inFlightOperations=new WeakSet();let pendingOperationScope=null;function announceOperation(message,state){operationNotice.textContent=message;operationNotice.dataset.state=state;operationNotice.hidden=false}function runOperation(scope){if(inFlightOperations.has(scope)){return false}inFlightOperations.add(scope);scope.setAttribute('aria-busy','true');scope.querySelectorAll('button,input,select,textarea').forEach((control)=>{control.dataset.operationWasDisabled=String(control.disabled);control.disabled=true});pendingOperationScope=scope;announceOperation('處理中，請稍候…','pending');queueMicrotask(()=>{if(pendingOperationScope===scope){pendingOperationScope=null;finishOperation(scope,false)}});return true}function finishOperation(scope,succeeded){scope.removeAttribute('aria-busy');scope.querySelectorAll('[data-operation-was-disabled]').forEach((control)=>{control.disabled=control.dataset.operationWasDisabled==='true';delete control.dataset.operationWasDisabled});inFlightOperations.delete(scope);announceOperation(succeeded?'操作已完成。':'操作失敗，請修正後重試。',succeeded?'success':'error')}const requestJSONWithoutOperationState=requestJSON;requestJSON=async function(url,options={}){const scope=pendingOperationScope;pendingOperationScope=null;try{const result=await requestJSONWithoutOperationState(url,options);if(scope){finishOperation(scope,true)}return result}catch(error){if(scope){finishOperation(scope,false)}throw error}};document.querySelectorAll('[data-node-form],[data-credential-form],[data-remote-form],[data-fallback-form],[data-network-form],[data-share-form]').forEach((form)=>form.addEventListener('submit',(event)=>{if(!runOperation(form)){event.preventDefault();event.stopImmediatePropagation()}},true))
`
